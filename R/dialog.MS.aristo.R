...
#
	tt <- tktoplevel()
	tkwm.title(tt,"MSeasyToARISTO ")	
#
# Variables for text fields
#

	
	msclustvar<-tclVar("Out_aristo-demo")
...
#
	...
	tkgrid(tklabel(TFrame,text="Export mass spectra to ARISTO format", font="Times 14", foreground="red"), labh)
	tkbind(labh, "<Button-1>", function() print(help("MSeasyToARISTO")))
	tkgrid(TFrame, sticky="we")
#
# MSeasyToARISTO

	IOFrame <- tkframe(tt, relief="groove", borderwidth=2)
	tkgrid(tklabel(IOFrame,text="- MSeasyToARISTO Input & Output files-", foreground="blue"), columnspan=5)
...
################################
	...
	#
		...
		substitute(MSeasyToARISTO(filename=paste(df)))
		}
		else
		{
		
		substitute(MSeasyToARISTO(filename=paste(df),cluster=as.integer(as.vector(as.numeric(unlist(strsplit(tclvalue(nctotvar),",")))))))
		}
